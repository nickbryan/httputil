...
func NewClient(options ...ClientOption) *Client {
	...
	for _, intercept := range opts.interceptors {
...
// It does not prepend basePath — the caller is responsible for the full URL.
// This is important for oapi-codegen users who construct their own requests.
...
func (c *Client) do(ctx context.Context, method, path string, body any, options ...RequestOption) (*http.Response, error) {
	...
	if opts.header.Get("Accept") == "" {
		req.Header.Set("Accept", c.codec.ContentType())
	}

	if bodyReader != nil && opts.header.Get("Content-Type") == "" {
