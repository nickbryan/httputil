...
// request bodies, decoding response bodies, and setting Content-Type and
// Accept headers.
...
// WithClientErrorDecoder sets the ErrorDecoder that the Client will use to
// decode error responses. It can be overridden per-request with
// WithRequestErrorDecoder.
func WithClientErrorDecoder(decoder ErrorDecoder) ClientOption {
	return func(co *clientOptions) {
		co.errorDecoder = decoder
	}
}
...
// WithHandlerMessages sets a custom [MessageFunc] that controls the user-facing
// error messages produced by validation failures. For [NewHandler], it
// customizes the messages in RFC 9457 constraint violation responses. For
// [NewFormHandler], it customizes the messages returned by [BindErrors.Get] and
// [BindErrors.All]. In both cases it applies to request body (Data) validation
// only — parameter validation messages are generated by the parameter binding
// pipeline.
func WithHandlerMessages(fn MessageFunc) HandlerOption {
...
// WithRequestErrorDecoder sets the ErrorDecoder for a single request,
// overriding any client-level ErrorDecoder.
func WithRequestErrorDecoder(decoder ErrorDecoder) RequestOption {
	return func(ro *requestOptions) {
		ro.errorDecoder = decoder
	}
}
...
// WithRequestParam adds a query parameter to the request. Parameters are merged
// with any query string already present in the path; if the same key appears in
// both, all values are preserved.
...
// WithRequestParams adds multiple query parameters to the request. Parameters
// are merged with any query string already present in the path; if the same key
// appears in both, all values are preserved.
